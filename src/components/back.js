...
class Back extends Component {
  ...
  componentDidMount() {
    ...
    .then((json) => {
      ...
      // this.setState({message: name + ", back the heck off."});
  ...
  componentWillShout() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      // this.setState({message: name.toUpperCase() + ", BACK THE HECK OFF!"});
      this.setState({message: message});
      this.setState({subtitle: subtitle});
    })
  }
  ...
  inGerman() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/back/${name}/${from}?shoutcloud&i18n=de`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inSpanish() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/back/${name}/${from}?i18n=es`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inFrench() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/back/${name}/${from}?i18n=fr`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inKorean() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/back/${name}/${from}?i18n=ko`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inRussian() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/back/${name}/${from}?i18n=ru`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inCanadian() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: name + ", take off hoser, eh?"});
