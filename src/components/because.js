...
class Because extends Component {
  constructor(props) {
    super(props);
    this.state = {
      value: '',
  ...
  componentDidMount() {
    ...
    fetch(`http://foaas.com/because/${from}`, {
      headers: {
  ...
  componentWillShout() {
    const from = this.state.from;
    fetch(`http://foaas.com/because/${from}?shoutcloud`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inGerman() {
    const from = this.state.from;
    fetch(`http://foaas.com/because/${from}?shoutcloud&i18n=de`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inSpanish() {
    const from = this.state.from;
    fetch(`http://foaas.com/because/${from}?i18n=es`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inFrench() {
    const from = this.state.from;
    fetch(`http://foaas.com/because/${from}?i18n=fr`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inKorean() {
    const from = this.state.from;
    fetch(`http://foaas.com/because/${from}?i18n=ko`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inRussian() {
    const from = this.state.from;
    fetch(`http://foaas.com/because/${from}?i18n=ru`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inCanadian() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: "Why?  Because I said so, eh?"});
