...
class Bucket extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`http://foaas.com/bucket/${from}`, {
      headers: {
  ...
  componentWillShout() {
    const from = this.state.from;
    fetch(`http://foaas.com/bucket/${from}?shoutcloud`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inGerman() {
    const from = this.state.from;
    fetch(`http://foaas.com/bucket/${from}?shoutcloud&i18n=de`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inSpanish() {
    const from = this.state.from;
    fetch(`http://foaas.com/bucket/${from}?i18n=es`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inFrench() {
    const from = this.state.from;
    fetch(`http://foaas.com/bucket/${from}?i18n=fr`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: "Il vous plaît étouffez sur un seau de coqs."});
      this.setState({subtitle: subtitle});
    })
  }
  ...
  inKorean() {
    const from = this.state.from;
    fetch(`http://foaas.com/bucket/${from}?i18n=ko`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inRussian() {
    const from = this.state.from;
    fetch(`http://foaas.com/bucket/${from}?i18n=ru`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inCanadian() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: "Please be careful eating that bucket of socks, eh?"});
