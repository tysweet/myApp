...
class No extends Component {
  ...
  componentDidMount() {
    ...
    .then((json) => {
      ...
      this.setState({message: "No figs given."});
  ...
  componentWillShout() {
    const from = this.state.from;
    fetch(`http://foaas.com/no/${from}?shoutcloud`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inGerman() {
    const from = this.state.from;
    fetch(`http://foaas.com/no/${from}?shoutcloud&i18n=de`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inSpanish() {
    const from = this.state.from;
    fetch(`http://foaas.com/no/${from}?i18n=es`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inFrench() {
    const from = this.state.from;
    fetch(`http://foaas.com/no/${from}?i18n=fr`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inKorean() {
    const from = this.state.from;
    fetch(`http://foaas.com/no/${from}?i18n=ko`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inRussian() {
    const from = this.state.from;
    fetch(`http://foaas.com/no/${from}?i18n=ru`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inCanadian() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: "Not a care in the Great White North, eh?"});
