...
class ShutUp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      value: '',
  ...
  componentDidMount() {
    ...
    fetch(`http://foaas.com/shutup/${name}/${from}`, {
      headers: {
    ...
    .then((json) => {
      ...
      this.setState({message: name + ", shut the fudge up."});
  ...
  componentWillShout() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/shutup/${name}/${from}?shoutcloud`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: name.toUpperCase() + ", SHUT THE FUDGE UP!"});
      this.setState({subtitle: subtitle});
    })
  }
  ...
  inGerman() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/shutup/${name}/${from}?shoutcloud&i18n=de`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inSpanish() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/shutup/${name}/${from}?i18n=es`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inFrench() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/shutup/${name}/${from}?i18n=fr`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inKorean() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/shutup/${name}/${from}?i18n=ko`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inRussian() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/shutup/${name}/${from}?i18n=ru`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inCanadian() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: name + ", you may not want to speak up right now, eh?"});
