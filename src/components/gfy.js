...
class Gfy extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`http://foaas.com/gfy/${name}/${from}`, {
      headers: {
  ...
  componentWillShout() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/gfy/${name}/${from}?shoutcloud`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inGerman() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/gfy/${name}/${from}?shoutcloud&i18n=de`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inSpanish() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/gfy/${name}/${from}?i18n=es`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inFrench() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/gfy/${name}/${from}?i18n=fr`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inKorean() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/gfy/${name}/${from}?i18n=ko`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inRussian() {
    const name = this.state.name;
    const from = this.state.from;
    fetch(`http://foaas.com/gfy/${name}/${from}?i18n=ru`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
  ...
  inCanadian() {
    ...
    .then((json) => {
      let message = json.message;
      let subtitle = json.subtitle;
      this.setState({message: "I'm not sure what this is supposed to mean " + name + ", but it sounds okay, eh?"});
